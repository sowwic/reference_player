...
class Window(QtWidgets.QMainWindow):
    ...
    def addMenuBar(self):
        ...
        ## PLAYBACK OPTIONS
        self.negatePlayBackStartAction = QtWidgets.QAction("Negate playback start")
        ...
        self.matchPlaybackOptionsAction = QtWidgets.QAction("Match player playback options")
    ...
    def createWidgets(self):
        ...
        self.backToStartButton.setIcon(self.style().standardIcon(QtWidgets.QStyle.SP_MediaSkipBackward))
        self.frameBackButton.setIcon(self.style().standardIcon(QtWidgets.QStyle.SP_MediaSeekBackward))
        self.playButton.setIcon(self.style().standardIcon(QtWidgets.QStyle.SP_MediaPlay))
        self.frameForwardButton.setIcon(self.style().standardIcon(QtWidgets.QStyle.SP_MediaSeekForward))
        self.toEndButton.setIcon(self.style().standardIcon(QtWidgets.QStyle.SP_MediaSkipForward))
    ...
    def createConnections(self):
        ...
        # Playback
        self.negatePlayBackStartAction.triggered.connect(self.negatePlayBackStart)
        self.setPlayBackStartAction.triggered.connect(self.setCurrentFrameAsStart)
        self.setPlayBackEndAction.triggered.connect(self.setCurrentFrameAsEnd)
        self.matchPlaybackOptionsAction.triggered.connect(self.setMayaPlaybackOptions)
        # View
    ...
    def setMayaTimeSlider(self, *args):
        if self.syncCheckBox.isChecked() and self.connected:
            if self.mayaClient.connect():
                self.mayaClient.setCurrentTime(int(self.playBackOffset.text()) +  self.timeSlider.value())
            else:
                self.timeSlider.valueChanged.disconnect()
                

    def openFile(self):
        fileName, _ = QtWidgets.QFileDialog.getOpenFileName(self, "Open Reference", QtCore.QDir.homePath())
        if fileName:
            ## STORE META DATA
    ...
    def durationChanged(self, duration):
        if duration:
            frameRates = [60 ,59.94, 50,48, 47.952, 40, 30, 29.97, 25, 24, 23.976, 20, 16, 15 , 12, 10, 8, 6, 5, 4, 3, 2]
            ...
            self.videoMeta.frameRate =  min(frameRates, key=lambda x:abs(x-rate))
            #Set media player update time
            self.mediaPlayer.setNotifyInterval(1/self.videoMeta.frameRate * 1000) 
    ...
    def getFrames(self, filePath):
        try:
            ...
            logger.exception("Failed to read file {0}".format(filePath), exc_info=1)
            raise e
    ...
    def setMayaPlaybackOptions(self):
        ...
        if self.mayaClient.connect() and self.videoMeta:
            ...
            # Set animation end
            cmd = "maya.cmds.playbackOptions(aet={0}, e=1)".format(self.videoMeta.frameCount)
            self.mayaClient.send(cmd)
            # Set playback start
            cmd = "maya.cmds.playbackOptions(min={0}, e=1)".format(float(self.playBackStart.text())+float(self.playBackOffset.text()))
            self.mayaClient.send(cmd)
            # Set playback end
            cmd = "maya.cmds.playbackOptions(max={0}, e=1)".format(float(self.playBackEnd.text())-float(self.playBackStart.text()))
            self.mayaClient.send(cmd)
        else:
            self.timeSlider.valueChanged.disconnect()
    ...
    def changeMayaPort(self):
        ...
        text, result = QtWidgets.QInputDialog.getText(self, "Maya port", "Set port: ", QtWidgets.QLineEdit.Normal, currentPort)
        if result:
            try:
    ...
    def showAbout(self):
        ...
        aboutDialog.setText("Author: Dmitrii Shevchenko\nVersion: {0}".format(self.version))
        aboutDialog.exec_()

    def showCommandPortHelp(self):
        helpDialog = QtWidgets.QMessageBox(self)
        helpDialog.setWindowTitle("How to connect to Maya")
        helpDialog.setText("Using Maya's script editor run the following command:")
        helpDialog.setInformativeText("maya.cmds.commandPort(name='127.0.0.1:7221', stp='python', echoOutput=True)")
        helpDialog.setDetailedText("You can change last four digits after : if you want different port opened.")
        helpDialog.exec_()
    ...
    def savePreset(self):
        ...
        fileName, extension = QtWidgets.QFileDialog.getSaveFileName(self, "Save Preset", QtCore.QDir.homePath(), "JSON (*.json)")
        with open(fileName, "w") as jsonFile:
            json.dump(preset, jsonFile, indent=4)
        self.statusBar.showMessage("Saved preset as {0}".format(fileName), 5000)


    def loadPreset(self):
        fileName, extension = QtWidgets.QFileDialog.getOpenFileName(self, "Load Preset", QtCore.QDir.homePath(), "JSON (*.json)")
        if fileName:
...
class MayaClient(object):
    ...
    def send(self, cmd):
        try:
            ...
            logger.exception("Failed to send command: {0}".format(cmd), exc_info=1)
            return None
        return self.recv()
...
if __name__ == '__main__':
    ...
    darkPalette.setColor(QtGui.QPalette.WindowText, QtGui.QColor(208, 208, 208))
    darkPalette.setColor(QtGui.QPalette.Base, QtGui.QColor(25, 25, 25))
    darkPalette.setColor(QtGui.QPalette.AlternateBase, QtGui.QColor(208, 208, 208))
    darkPalette.setColor(QtGui.QPalette.ToolTipBase, QtGui.QColor(208, 208, 208))
    darkPalette.setColor(QtGui.QPalette.ToolTipBase, QtGui.QColor(208, 208, 208))
    ...
    darkPalette.setColor(QtGui.QPalette.ButtonText, QtGui.QColor(208, 208, 208))
